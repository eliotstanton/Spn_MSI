...
then
    ...
    echo "No sequence data directory path argument given. (-s)"
    exit 1
fi
...
then
    ...
    echo "No reference database directory path argument given. (-r)"
    exit 1
fi
...
do
	...
	var_sample=$(echo "$readPair_1" | awk -F[/_] '{print $(NF-4)}')

	printf "sbatch -p small --cpus-per-task 8 --time=4:00:00 --output $out_qsub$var_sample.out \
--error $out_qsub$var_sample.err StrepLab-JanOw_SPN-Typer.sh $var_job $out_jobCntrl\n"
	...
	sbatch \
		...
		--cpus-per-task 8 \
		--time=4:00:00 \
		--output $out_qsub$var_sample.out \
		--error $out_qsub$var_sample.err \
		StrepLab-JanOw_SPN-Typer.sh \
		$var_job \
		$out_jobCntrl
	printf "\n"
...
#var_count='wc -l $out_jobCntrl/job-control.txt'
